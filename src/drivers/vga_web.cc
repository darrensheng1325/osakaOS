...
void VideoGraphicsArray::DrawLine(int32_t x0, int32_t y0, 
	...
	if (os::math::abs(y1 - y0) < os::math::abs(x1 - x0)) {
...
//actually draw to the screen — for the web build we push the indexed
//framebuffer to JavaScript which renders it to a canvas.
void VideoGraphicsArray::DrawToScreen() {
#ifdef __EMSCRIPTEN__
	if (!pixels) return;
	EM_ASM_({
		if (Module.drawFrameBuffer) {
			Module.drawFrameBuffer($0, $1, $2);
		}
	}, (unsigned int)pixels, gfxWidth, gfxHeight);
#else
